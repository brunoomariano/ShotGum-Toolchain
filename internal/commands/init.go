...
func initCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println(styles.DescStyle.Render("  1. Add a script folder:   stg add folder ~/my-scripts --name mycat"))
			fmt.Println(styles.DescStyle.Render("  2. Add a script:          stg add script ~/my-scripts/deploy.sh --category mycat"))
